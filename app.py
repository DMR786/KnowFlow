...
# Set your secret key here. It can be a random string.
...
app.config['MAIL_USERNAME'] = os.getenv('EMAIL_USER')  # Set in environment
app.config['MAIL_PASSWORD'] = os.getenv('EMAIL_PASS')  # Set in environment
app.config['MAIL_DEFAULT_SENDER'] = os.getenv('EMAIL_USER')  # Set in environment
...
def send_email_route():
    ...
    if email:
        success = send_email(query, stackoverflow_results, reddit_results, email)
        if success:
            flash('Email sent successfully!', 'success')
        else:
            flash('Failed to send email. Please check your email settings.', 'danger')
    else:
        flash('Email not provided. Results were not sent.', 'warning')

    return redirect(url_for('email_sent', email=email))  # Redirect to the email_sent route
